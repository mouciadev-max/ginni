...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [prodRes, catRes, userRes, orderRes] = await Promise.all([
        ...
        axios.get('/api/admin/users', { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } }),
        axios.get('/api/admin/orders', { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } })
      ]);
  ...
  const handleAddCategory = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/api/categories', newCategory, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
      setNewCategory({ name: '' });
    } catch (error) { 
      console.error(error);
  ...
  const handleAddSubcategory = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`/api/categories/${newSubcategory.categoryId}/subcategories`, { name: newSubcategory.name }, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
  ...
  const handleDeleteCategory = async (id) => {
    ...
    try {
      await axios.delete(`/api/categories/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteSubcategory = async (id) => {
    ...
    try {
      await axios.delete(`/api/categories/subcategories/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
    } catch (error) {
      console.error(error);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      ...
      if (editingProductId) {
        await axios.put(`/api/products/${editingProductId}`, formData, config);
      } else {
  ...
  const handleDeleteProduct = async (id) => {
    try {
      await axios.delete(`/api/products/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
    } catch (error) { 
      console.error(error);
  ...
  const handleDeleteUser = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/users/${id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.patch(`/api/admin/orders/${orderId}/status`, { status: newStatus }, { headers: { Authorization: `Bearer ${localStorage.getItem('accessToken')}` } });
      fetchData();
    } catch (error) {
      console.error(error);
